...
`CREATE OR REPLACE TABLE … AS SELECT` and returns a table name. The one exception, training,
is named in [code-structure.md](code-structure.md) with the reason.
...
| Workbench                            | Nothing in this repository is notebook-shaped. The analysis is a separate repository that runs locally against a local file, so a managed notebook would put an always-on VM between an analyst and something they already have. |
