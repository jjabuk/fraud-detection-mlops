...
`analysis/`, against a permuted null. What is new is that the analysis now checks its
...
statistics and moved to `analysis/`, an R package with its own dependency graph, tests and
...
1. *Two copies of the policy.* The graph and the notebooks each carried their own
   thresholds. Every threshold now lives in `analysis/R/policy.R` and is sourced by both —
...
fraud rates (`W` at 0.0198 over 77% of rows, `C` at 0.1467 over 11%). Measured on the
admitted set: `card3` scores 0.656 pooled and **0.501** inside `W`; `M4` 0.653 and 0.501.
A coin flip on three quarters of the traffic, credited as signal by every pooled check.
...
**What survives.** The measurement. The fragment records `would_reject: 26` and that **262
of 502 columns are unmeasurable inside a segment holding 74% of rows**, which is a real
coverage finding. Deleting the audit would have thrown that away with the inference.
...
first transaction, so `day − D1` is their account-start day. Judged by label purity, which
is the right test precisely because fraud propagates within a client:

| Entity          | Entities | Label-pure groups | Fraud share when the group has any fraud |
| --------------- | -------- | ----------------- | ---------------------------------------- |
| `card1`         | 13,553   | 84.8%             | 18.0%                                    |
| `card1 + addr1` | 39,974   | 89.0%             | 25.0%                                    |
| client uid      | 217,735  | **96.6%**         | **61.2%**                                |
...
**Where it runs.** As a one-off offline analysis, not as a pipeline stage. The V-block is
